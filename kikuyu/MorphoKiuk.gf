...
--Kikuyu Resource Morphology

-- Benson Kituku 2017 -- 2018

resource MorphoKiuk = CommonBantu ,ResKiuk ** open Prelude, Predef
in {
...
Detsomeplprefix : Gender ->  Str = \g ->
 ...
 mkNum : Str -> Str -> Str -> Str -> {s : DForm => CardOrd => Gender => Str} = 
    ...
    {s = table {
       ...
       ten  => table {NCard =>\\g =>"mirongo "  ++ twelve ; 
                      NOrd => \\g => Ordprefix g ++ "mirongo" ++ twelve};
       hund  => table {NCard =>\\g =>"magana "  ++ twenty ; 
                      NOrd => \\g => Ordprefix g ++ "magana" ++ twenty}  
       }
    } ;
   ...
   mkNumn : Str -> Str -> Str -> Str -> {s : DForm => CardOrd => Gender => Str} = 
    ...
    {s = table {
       ...
       hund  => table {NCard =>\\g =>"igana "  ++ twenty ; 
                      NOrd => \\g => Ordprefix g ++ "igana" ++ twenty}  
       }
    } ;
  ...
  regNum : Str -> {s : DForm => CardOrd => Gender => Str} = 
    \six -> {s = table {
       ...
       ten  => table {NCard =>\\g =>"mirongo "  ++ six ; 
                      NOrd => \\g => Ordprefix g ++ "mirongo" ++ six};
       hund  => table {NCard =>\\g =>"magana "  ++ six ; 
                      NOrd => \\g => Ordprefix g ++ "magana" ++ six}  
       } } ;
...
--regular expression for adjective colours
 cregA : Str->  {s : AForm =>  Str} = \seo -> {  
       s = table {
             AAdj g Sg => ProunSgprefix g + "a" ++"rangi" ++"wa"  ++ seo; 
             AAdj g Pl=> ProunPlprefix g + "a" ++"rangi" ++"wa"  ++ seo;
            AComp g n =>  ""} } ;
...
regV : Str -> Verb = 
     ...
     s = table{  True => table{
       ...
       VPast g n p => Verbprefix g n p + init vika + "ia" ;
